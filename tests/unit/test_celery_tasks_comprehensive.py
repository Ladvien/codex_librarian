...
class TestGenerateEmbeddings:
    ...
    async def test_generate_embeddings_service_error(self):
        ...
        with patch('src.pdf_to_markdown_mcp.worker.tasks.get_db_session') as mock_db, \
            ...
            mock_embedding_service.generate_batch.side_effect = EmbeddingError("API failed")
            mock_embedding_class.return_value = mock_embedding_service
...
class TestBatchProcessPdfs:
    ...
    async def test_batch_process_pdfs_success(self):
        ...
        with patch('src.pdf_to_markdown_mcp.worker.tasks.process_pdf_document') as mock_process:
...
class TestCleanupOldFiles:
    ...
    def test_cleanup_old_files_success(self, mock_isfile, mock_remove, mock_getmtime, mock_listdir):
    ...
    @patch('os.path.isfile')
    @patch('os.path.getmtime')
    def test_cleanup_old_files_permission_error(self, mock_getmtime, mock_isfile, mock_remove, mock_listdir):
...
class TestMonitorWorkerHealth:
    ...
    def test_monitor_worker_health_success(self, mock_disk, mock_memory, mock_cpu):
        ...
        with patch('src.pdf_to_markdown_mcp.worker.tasks.get_db_session') as mock_db, \
            ...
            mock_app.control.inspect.return_value.stats.return_value = {'worker1': mock_stats}
    ...
    def test_monitor_worker_health_high_resource_usage(self, mock_cpu):
        ...
        with patch('psutil.virtual_memory') as mock_memory, \
            ...
            mock_memory.return_value = Mock(percent=90.0, available=1000000)  # High memory
    ...
    def test_monitor_worker_health_database_error(self):
        ...
        assert any('database' in warning.lower() for warning in result.get('warnings', []))