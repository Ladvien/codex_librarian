...
class TestMinerUService:
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_success(self, mineru_service, sample_pdf_path, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.MinerUAPI') as mock_api:
            ...
            # When (Act)
            result = await mineru_service.process_pdf(sample_pdf_path, processing_options)
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_invalid_file_type(self, mineru_service, processing_options):
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_with_tables(self, mineru_service, sample_pdf_path, processing_options):
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_with_formulas(self, mineru_service, sample_pdf_path, processing_options):
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_with_images(self, mineru_service, sample_pdf_path, processing_options):
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_chunking_enabled(self, mineru_service, sample_pdf_path, processing_options):
    ...
    @pytest.mark.asyncio
    async def test_process_pdf_timeout_error(self, mineru_service, sample_pdf_path, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.MinerUAPI') as mock_api:
            mock_instance = Mock()
            mock_api.return_value = mock_instance
            mock_instance.process_pdf = AsyncMock(side_effect=asyncio.TimeoutError())
    ...
    @pytest.mark.asyncio
    async def test_production_mode_fails_when_mineru_unavailable(self, sample_pdf_path, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.settings') as mock_settings:
            ...
            with patch('pdf_to_markdown_mcp.services.mineru.MinerUAPI', None):
                ...
                assert "MinerU library not available in production" in str(exc_info.value)
                assert "DEPENDENCY_MISSING" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_development_mode_allows_mock_processing(self, sample_pdf_path, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.settings') as mock_settings:
            ...
            with patch('pdf_to_markdown_mcp.services.mineru.MinerUAPI', None):
                ...
                with patch.object(service, 'validate_pdf_file', new_callable=AsyncMock, return_value=True), \
                     patch('pathlib.Path.stat') as mock_stat:

                    mock_stat.return_value.st_size = 1024 * 1024  # 1MB

                    # When (Act)
                    result = await service.process_pdf(sample_pdf_path, processing_options)

                    # Then (Assert)
                    assert result is not None
                    assert "mock" in result.processing_metadata.get("mineru_version", "").lower()
    ...
    async def test_file_size_streaming_threshold_enforced(self, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.settings') as mock_settings:
            ...
            with patch.object(service, 'validate_pdf_file', new_callable=AsyncMock, return_value=True), \
                 patch('pathlib.Path.stat') as mock_stat, \
                 patch.object(service, '_process_with_mineru_streaming', new_callable=AsyncMock) as mock_streaming:
    ...
    async def test_memory_safe_hash_calculation(self, processing_options):
        ...
        with patch('pdf_to_markdown_mcp.services.mineru.settings') as mock_settings:
            ...
            with patch('pathlib.Path.stat') as mock_stat, \
                ...
                mock_file.read.side_effect = [b'chunk1', b'chunk2', b'']  # End with empty to stop iteration
