#!/usr/bin/env python3
"""
Demo script showing file watcher functionality without database dependencies.

This script demonstrates the core file watching capabilities using a mock task queue
that doesn't require database or Celery dependencies.
"""
...
class DemoTaskQueue:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_processed': self.processed_files[-1] if self.processed_files else None
        }
...
class DemoWatcherService:
    ...
    def create_demo_directory(self, base_dir: str = "./demo_watch") -> Path:
        ...
        logger.info(f"   Or into subdirectories: {demo_path / 'pdfs'}, {demo_path / 'input'}")
...
%%EOF"""
    ...
    def run_demo_loop(self):
        ...
        logger.info("💡 Try adding PDF files to the watch directories to see them detected!")
    ...
    def shutdown(self):
        ...
        if stats['total_files'] > 0:
            ...
            for i, file_info in enumerate(self.task_queue.processed_files[-5:], 1):  # Show last 5
                logger.info(f"      {i}. {Path(file_info['file_path']).name} (#{file_info['document_id']})")
