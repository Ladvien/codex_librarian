...
class CallbackTask(Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        logger.info(
            ...
            extra={
                'task_id': task_id,
                'task_name': self.name,
                'result': retval if isinstance(retval, (dict, str, int)) else type(retval).__name__
            }
        )
    ...
    def _categorize_error(self, exc) -> str:
        ...
        if exc_name in ['ConnectionError', 'TimeoutError', 'RedisError', 'DatabaseError']:
            return 'transient'
...
def create_celery_app() -> Celery:
    ...
    app.conf.update(
        ...
        broker_transport_options={
            ...
            'max_connections': min(settings.celery.redis_max_connections, 15),  # Reduced from 20
        ...
        result_backend_transport_options={
            ...
            'max_connections': max(5, min(settings.celery.redis_max_connections // 2, 10)),
            'socket_timeout': settings.celery.redis_socket_timeout,
            'socket_connect_timeout': settings.celery.redis_socket_connect_timeout,
            'retry_on_timeout': settings.celery.redis_retry_on_timeout,
            'health_check_interval': settings.celery.redis_health_check_interval,
    ...
    return [
        ...
        Queue(
            ...
            queue_arguments={
                ...
                'x-message-ttl': settings.celery.task_message_ttl * 1000,  # Convert to ms
        ...
        Queue(
            ...
            queue_arguments={
                'x-max-priority': 5,
                'x-message-ttl': (settings.celery.task_message_ttl // 2) * 1000,  # 30 minutes TTL
                'x-max-length': 5000,
        ...
        Queue(
            ...
            queue_arguments={
                'x-max-priority': 1,
                'x-message-ttl': (settings.celery.task_message_ttl * 2) * 1000,  # 2 hours TTL
                'x-max-length': 1000,
...
def task_postrun_handler(sender=None, task_id=None, task=None, args=None, kwargs=None, retval=None, state=None, **kwds):
    ...
    logger.info(
        f"Completed task {task.name} [{task_id}] in {duration:.2f}s" if duration
        ...
        try:
            with app.connection() as conn:
                ...
                queue_stats[queue_name] = {
                    ...
                    'overflow_behavior': queue.queue_arguments.get('x-overflow', 'none'),
...
def get_worker_health() -> Dict[str, Any]:
    ...
    try:
        ...
        total_queued = sum(
            stat.get('length', 0) for stat in queue_stats.values()
        ...
        critical_queued = sum(
            stat.get('length', 0) for queue_name, stat in queue_stats.items()
            if queue_name in critical_queues and isinstance(stat, dict) and 'length' in stat
        )

        if critical_queued > 100:  # Critical threshold
            health_status['checks']['queue_depth'] = f'critical: {critical_queued} critical tasks queued'
            health_status['status'] = 'unhealthy'
        elif total_queued > 1000:  # General threshold
            health_status['checks']['queue_depth'] = f'warning: {total_queued} tasks queued'
            if health_status['status'] == 'healthy':
                health_status['status'] = 'degraded'
        else:
            health_status['checks']['queue_depth'] = f'healthy: {total_queued} tasks queued'
    ...
    try:
        ...
        if 'pool_utilization_percent' in redis_info:
            ...
            if utilization > 90:
                health_status['checks']['connection_pool'] = f'critical: {utilization}% utilized'
                health_status['status'] = 'unhealthy'
            elif utilization > 75:
                health_status['checks']['connection_pool'] = f'warning: {utilization}% utilized'
                if health_status['status'] == 'healthy':
                    health_status['status'] = 'degraded'
            else:
                health_status['checks']['connection_pool'] = f'healthy: {utilization}% utilized'
