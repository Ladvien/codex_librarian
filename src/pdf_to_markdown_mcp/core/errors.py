...
def get_retry_strategy(error: Exception) -> RetryConfig:
    ...
    if category == ErrorCategory.VALIDATION or category == ErrorCategory.SECURITY:
        ...
        return RetryConfig(max_retries=3, base_delay=1.0, max_delay=30.0)
    elif category == ErrorCategory.RESOURCE:
        return RetryConfig(max_retries=5, base_delay=5.0, max_delay=120.0)
    elif category == ErrorCategory.EXTERNAL:
        return RetryConfig(max_retries=3, base_delay=2.0, max_delay=60.0)
    else:
        return RetryConfig(max_retries=0)
