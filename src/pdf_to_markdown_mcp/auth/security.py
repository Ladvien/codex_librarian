...
ALLOWED_PATHS = [
    ...
    os.environ.get("INPUT_DIRECTORY", "/mnt/codex_fs/research/"),
    ...
    try:
        ...
        if not allowed:
            logger.error(
                ...
                extra={"path": str(path), "resolved": str(resolved_path), "allowed": ALLOWED_PATHS}
            )
        ...
        dangerous_patterns = [
            "/etc/", "/root/", "/home/", "/usr/bin/", "/bin/",
            "passwd", "shadow", ".ssh", ".env", "config"
        ]
...
async def verify_api_key_dependency(
    ...
    if not credentials:
        logger.warning(f"Authentication failed: No credentials provided from {client_ip}")
        raise HTTPException(
    ...
    if not secrets.compare_digest(provided_key, API_KEY):
        ...
        logger.warning(
            ...
            extra={
                ...
                "provided_key_hash": hashlib.sha256(provided_key.encode()).hexdigest()[:8]
            }
        )
...
def sanitize_error_message(error_message: str) -> str:
    ...
    if any(word in sanitized.lower() for word in ['password', 'secret', 'token', 'key']):
